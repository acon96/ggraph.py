...
- [x] make the pip packge install the ggml shared libraries from source so that the compiled version is as optimized as possible (`GGML_NATIVE=ON`)  
...
- [ ] advanced tensor allocation/graph routing for heterogeneus compute setups (i.e. multiple nodes using RPC backend)